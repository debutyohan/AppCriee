...
namespace AppCriée
{
    public partial class AppCriee_DirecteurDesVentes : Form
    {
        ...
        private void btn_directeurdesventes_mesdonnees_validermodif_Click(object sender, EventArgs e)
        {
            ...
            CURS csb = new CURS();
            csb.ReqSelectPrepare("SELECT COUNT(login) as nblogin FROM Utilisateur WHERE login=? AND id!=?", new List<object> { (new CryptData(tbx_directeurdesventes_mesdonnees_login.Text)).EncryptData(), idUserModified });
            string nblogin = csb.champ("nblogin").ToString();
            csb.suivant();
            csb.fermer();
            if (nblogin == "1")
            {
                lbl_directeurdesventes_mesdonnees_validationmodiferreur.Text = "Le login est déjà utilisé pour un utilisateur";
                lbl_directeurdesventes_mesdonnees_validationmodiferreur.Show();
                return;
            }
            CURS csa = new CURS();
            csa.ReqSelectPrepare("SELECT COUNT(adrMail) as nbadrMail FROM Utilisateur WHERE adrMail=? AND id!=?", new List<object> { (new CryptData(tbx_directeurdesventes_mesdonnees_adrMail.Text)).EncryptData(), idUserModified });
            string nbadrMail = csa.champ("nbadrMail").ToString();
            csa.suivant();
            csa.fermer();
            if (nbadrMail == "1")
            {
                lbl_directeurdesventes_mesdonnees_validationmodiferreur.Text = "L’adresse mail est déjà utilisé pour un utilisateur";
                lbl_directeurdesventes_mesdonnees_validationmodiferreur.Show();
                return;
            }
            csa.suivant();
            ...
            CURS cs = new CURS();
            cs.ReqAdminPrepare("UPDATE Utilisateur SET login=?, nomuser=?, prenomuser=?, adrMail=? WHERE id=?", new List<object> { (new CryptData(tbx_directeurdesventes_mesdonnees_login.Text)).EncryptData(), (new CryptData(nomModif)).EncryptData(), (new CryptData(prenomModif)).EncryptData(), (new CryptData(adrMail)).EncryptData(), idUserModified });
            cs.fermer();
